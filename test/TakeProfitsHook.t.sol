...
contract TakeProfitsTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        (_key, ) = initPool(
            _tokenOne,
            _tokenTwo,
            _hook, // Hook address is passed here
            3000,
            _SQRT_RATIO_1_1
        );
    ...
    function test_cancelOrder() public {
        ...
        // Cancel the order
        _hook.cancelOrder(_key, tickLower, zeroForOne);

        // Check that we received our token0 tokens back, and no longer own any ERC-1155 tokens
        uint256 finalBalance = _tokenOne.balanceOfSelf();
        assertEq(finalBalance, originalBalance);

        tokenBalance = _hook.balanceOf(address(this), tokenId);
    ...
    function test_orderExecute_zeroForOne() public {
        ...
        IPoolManager.SwapParams memory params = IPoolManager.SwapParams({
            ...
            sqrtPriceLimitX96: _SQRT_RATIO_1_1 - 1
        });
        ...
        int256 tokensLeftToSell = _hook.takeProfitPositions(
            _key.toId(),
            tick,
            zeroForOne
        );
        ...
        uint256 tokenId = _hook.getTokenId(_key, tickLower, zeroForOne);
        uint256 claimableTokens = _hook.tokenIdClaimable(tokenId);
        uint256 hookContractToken1Balance = _tokenTwo.balanceOf(address(_hook));
        assertEq(claimableTokens, hookContractToken1Balance);

        // Ensure we can redeem the token1 tokens
        uint256 originalToken1Balance = _tokenTwo.balanceOf(address(this));
        _hook.redeem(tokenId, amount, address(this));
        uint256 newToken1Balance = _tokenTwo.balanceOf(address(this));
